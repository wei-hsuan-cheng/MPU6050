...
// Macro to check return values; if error, jump into an error loop
...
// // (Optional) Timer callback used by the micro-ROS executor
// // In this example we use polling in loop so the timer callback can be empty.
...
void setup_micro_ros() {
  ...
  // Create a publisher for geometry_msgs/msg/Quaternion on topic "imu_quaternion"
  ...
  // // (Optional) Create a timer and add it to an executor if you need to handle subscriptions or other callbacks.
  // const unsigned int timer_timeout = 33; // [ms]
...
void loop() {
  // Non-blocking spin to process callbacks as soon as they are available
